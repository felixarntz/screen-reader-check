...
class ImagesAlternativeTexts extends Test {
	...
	protected function run( $result, $dom ) {
		...
		foreach ( $images as $image ) {
			...
			if ( null === $alt ) {
				$result['message_codes'][] = 'error_missing_alt_attribute';
				...
				if ( is_string( $src ) ) {
					...
					if ( $image_type ) {
						if ( 'content' === $image_type ) {
							$result['message_codes'][] = 'error_empty_alt_attribute_content';
							...
							if ( $title ) {
								$result['message_codes'][] = 'warning_usage_of_title_attribute_layout';
				...
				if ( is_string( $src ) ) {
					if ( false !== strpos( $src, $alt ) ) {
						$result['message_codes'][] = 'error_alt_attribute_part_of_src';
						...
						if ( in_array( trim( strtolower( $alt ) ), $blacklist ) ) {
							...
							if ( 80 < strlen( $alt ) ) {
								$result['message_codes'][] = 'error_alternative_text_too_long';
